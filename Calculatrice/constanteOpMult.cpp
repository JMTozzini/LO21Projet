...
{
    return *(new Rationnel(numerateur * ra.GetNum(),ra.GetDen() * denominateur));
}
Rationnel& Rationnel::operator*(const Entier& e)
{
    return *(new Rationnel(numerateur * e.GetVal(),denominateur));
}
...
{
    return *(new Rationnel(ra.GetNum() * valeur, ra.GetDen()));
}
